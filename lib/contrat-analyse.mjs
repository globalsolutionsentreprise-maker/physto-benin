...
// Planning d'un contrat. Reproduit le motif réellement en base sur le dossier
// La Manne Dorée : une intervention tous les 92 jours (365/4 arrondi au
// supérieur), et un contrôle 45 jours après chacune. Le contrôle n'est posé que
// si l'intervalle le permet, sinon il chevaucherait la visite suivante.
export const JOURS_AVANT_CONTROLE = 45
...
export function datesPassages({ dateDebut, dureeMois, frequence }) {
  ...
  for (let i = 0; i < nb; i++) {
    ...
    if (pas > JOURS_AVANT_CONTROLE) {
      out.push({ date: isoPlusJours(dateDebut, i * pas + JOURS_AVANT_CONTROLE), type: "controle" })
    }
  }
