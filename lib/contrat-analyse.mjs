...
// Un contrat est "suivi par passages" dès qu'un de ses passages porte un montant
// dû : dans ce cas paiements_recus est la somme des paiements des passages, et
// aucun autre écrivain (étape encaissement du parcours, backfill) ne doit l'écraser.
export function paiementsParPassages(interventions) {
  return (interventions || []).some(i => (Number(i.montant_du) || 0) > 0)
}
...
// État de paiement d'un passage, dérivé (jamais stocké). Passe en "alerte" si
// l'échéance approche (date ≤ auj + seuil) sans être soldée, ou si un passage
// antérieur reste impayé.
...
export function resumeContrat({ devis, interventions }, aujourdhui) {
  ...
  // Encaissement : statut de paiement par passage (dérivé) + agrégats contrat.
  // Un passage antérieur non soldé fait passer les suivants en alerte.
