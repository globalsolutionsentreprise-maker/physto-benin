...
[2026-09-22] | Contrat réédité réaffichait la note « intervention initiale (devis X) facturée séparément » : `ouvrirContratExistant` faisait `sansNoteDevis: p.sansNoteDevis ? "1" : "0"`, donc param absent/false → note réaffichée, alors que le contrat prend le dessus sur le devis (grille contrat déjà dérivée du devis = double facturation). | Un contrat une fois établi/réédité prime sur le devis : `ouvrirContratExistant` force `sansNoteDevis:"1"`. La note du devis initial reste opt-in UNIQUEMENT à la 1re proposition (case du modal), jamais à la réédition.
