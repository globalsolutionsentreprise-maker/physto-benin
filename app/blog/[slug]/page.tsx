...
// Pré-génération statique des articles au build + revalidation horaire (ISR).
// Un nouvel article non pré-généré est rendu à la demande (dynamicParams par défaut).
...
const getAllArticles = cache(async function (): Promise<any[]> {
  try {
    ...
    console.error("getAllArticles: erreur Supabase", e)
    return []
  }
})
...
export async function generateStaticParams() {
  ...
  return articles
    .filter((a: any) => a.titre)
    .map((a: any) => ({ slug: slugifier(a.titre) }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params
  const article = (await getAllArticles()).find((a: any) => slugifier(a.titre) === slug) || null
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const articles = await getAllArticles()
  const article = articles.find((a: any) => slugifier(a.titre) === slug) || null

  if (!article) notFound()

  const autres = articles
    .filter((a: any) => slugifier(a.titre) !== slug && a.contenu && a.contenu.trim() !== "")
    .slice(0, 3)
              ...
              <div style={{ backgroundColor: "#f7f7f5", border: "1px solid #e8e8e8", borderLeft: "3px solid #d4a920", borderRadius: "8px", padding: "24px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
                <div>
                  ...
                  <div style={{ fontSize: "15px", fontWeight: "600", color: "#0a2e1a" }}>
                    Découvrir notre service de {service.label} au Bénin
                  </div>
                </div>
