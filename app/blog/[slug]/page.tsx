    ...
    async function charger() {
      ...
      if (data) {
        const trouve = data.find(function(a) {
          return slugifier(a.titre) === params.slug
        })
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          {article.contenu ? (
            <div style={{ fontSize: "16px", color: "#333", lineHeight: "1.9" }}>
              {article.contenu.split("\n").map(function(paragraphe: string, i: number) {
                if (!paragraphe.trim()) return <br key={i} />
                if (paragraphe.startsWith("# ")) {
                  return <h2 key={i} style={{ fontSize: "24px", fontWeight: "700", color: "#0a0a0a", marginTop: "40px", marginBottom: "16px" }}>{paragraphe.replace("# ", "")}</h2>
                }
                if (paragraphe.startsWith("## ")) {
                  return <h3 key={i} style={{ fontSize: "18px", fontWeight: "700", color: "#0a0a0a", marginTop: "32px", marginBottom: "12px" }}>{paragraphe.replace("## ", "")}</h3>
                }
                if (paragraphe.startsWith("- ")) {
                  return <li key={i} style={{ fontSize: "15px", color: "#444", marginBottom: "8px", marginLeft: "20px" }}>{paragraphe.replace("- ", "")}</li>
                }
                return <p key={i} style={{ marginBottom: "20px" }}>{paragraphe}</p>
              })}
            </div>
          ) : (
            <div>
              <p style={{ fontSize: "16px", color: "#555", lineHeight: "1.9", marginBottom: "24px" }}>
                {article.resume}
              </p>
